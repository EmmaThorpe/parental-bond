...
class ProjectSampleRandom {
    ...
    randomChoice(options) {
        var choice, checkedChoices = [];
        for (choice in options) {
            // always chooses a random move (never switches)
            if (options.hasOwnProperty(choice) && !choice.startsWith("switch")){
                console.log(choice);
                checkedChoices.push(choice);
            }
        }
        return checkedChoices[Math.floor(Math.random() * checkedChoices.length)];
    }
